...
### Still not built
No auth, Profile page (still a placeholder), import pipeline, no book edit or delete, no actual LLM behind the AI Librarian. Dashboard, Quotes, Search, AI Librarian, Analytics, Reading, Timeline, and Notes still read `MOCK_BOOKS` — and several of these (Quotes, Search, AI Librarian) depend on `getBookDetail()`'s hash-derived `favoriteQuote`, so wiring them needs the same "don't trust the hash for DB books" care described above, likely via a real `Quote` table query rather than reusing `getBookDetail()`. This is still an early slice of an 11-phase spec — expect this to take many more sessions, similar to how her NEXORA Finance OS project took ~24 sessions.
