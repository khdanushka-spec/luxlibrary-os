...
### Still not built
No auth, Profile page (still a placeholder), import pipeline, no book edit or delete, no actual LLM behind the AI Librarian, search doesn't cover notes. Dashboard/Wishlist/Collections/Authors/Genres/Publishers/Series/Quotes/Search/AI Librarian/Analytics/Reading/Timeline/Notes all still read `MOCK_BOOKS` instead of the real (now-live) database — that's the natural next chunk of work. This is still an early slice of an 11-phase spec — expect this to take many more sessions, similar to how her NEXORA Finance OS project took ~24 sessions.
