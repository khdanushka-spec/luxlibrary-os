...
### Still not built
No real database (Neon isn't provisioned — Storage tab in Vercel dashboard, or ask to set one up next), no auth, Publishers/Series/Notes/Timeline/Settings/Profile pages (still placeholders), import pipeline, no book edit or delete, no actual LLM behind the AI Librarian, search doesn't cover notes (none exist yet). This is still an early slice of an 11-phase spec — expect this to take many more sessions, similar to how her NEXORA Finance OS project took ~24 sessions.
