...
### Still not built
No auth, Profile page (still a placeholder), import pipeline, no book edit or delete, no actual LLM behind the AI Librarian. Dashboard, AI Librarian, Analytics, Reading, Timeline, and Notes still read `MOCK_BOOKS`. AI Librarian in particular still calls `getBookDetail()` for "summarize"/"where is" answers — same hash-derived-data risk as before, needs converting to real async DB queries (bigger lift than the other pages since the whole rule engine in `lib/ai-librarian.ts` is currently synchronous). This is still an early slice of an 11-phase spec — expect this to take many more sessions, similar to how her NEXORA Finance OS project took ~24 sessions.
